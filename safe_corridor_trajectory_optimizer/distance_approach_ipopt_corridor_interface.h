...
private:
  //***************    start ADOL-C part ***********************************
  double *obj_lam = nullptr;
  //** variables for sparsity exploitation
  unsigned int *rind_g = nullptr; /* row indices 雅克比矩阵行记录    */
  unsigned int *cind_g = nullptr; /* column indices 雅克比矩阵列记录*/
  double *jacval = nullptr;       /* values  雅克比矩阵的列     */
  unsigned int *rind_L = nullptr; /* row indices  拉格朗日矩阵行  */
  unsigned int *cind_L = nullptr; /* column indices 拉格朗日列 */
  double *hessval = nullptr;      /* values  拉格朗日值*/
  int nnz_jac = 0;          //约束的雅克比矩阵非零数
  int nnz_L = 0;            //海森矩阵非零数
  int options_g[4] = {0, 0, 0, 0};
  int options_L[4] = {0, 0, 0, 0};
  //***************    end   ADOL-C part ***********************************
};
