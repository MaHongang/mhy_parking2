/******************************************************************************
 * Copyright 2018 The Apollo Authors. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *****************************************************************************/

/*
 * @file
 */

#include "coarse_trajectory_generator/node3d.h"
#include "absl/strings/str_cat.h"

using common::math::Box2d;

// 基本构造：只指定 x, y, phi
Node3d::Node3d(double x, double y, double phi) {
  x_ = x;
  y_ = y;
  phi_ = phi;
}

// 构造：使用配置计算网格索引并初始化 traversed_* 为单点轨迹
Node3d::Node3d(double x, double y, double phi,
               const std::vector<double> &XYbounds,
               const PlannerOpenSpaceConfig &open_space_conf) {
  // XYbounds 应为 {xmin, xmax, ymin, ymax}

  x_ = x;
  y_ = y;
  phi_ = phi;

  // 根据配置中的栅格分辨率计算离散化的网格索引
  x_grid_ = static_cast<int>((x_ - XYbounds[0]) /
                       open_space_conf.warm_start_config.xy_grid_resolution);
  y_grid_ = static_cast<int>((y_ - XYbounds[2]) /
                       open_space_conf.warm_start_config.xy_grid_resolution);
  phi_grid_ = static_cast<int>(
      (phi_ - (-M_PI)) / open_space_conf.warm_start_config.phi_grid_resolution);

  // 将该点作为 traversed_* 的第一个元素，表示从父节点到当前节点至少包含此点
  traversed_x_.push_back(x);
  traversed_y_.push_back(y);
  traversed_phi_.push_back(phi);

  index_ = ComputeStringIndex(x_grid_, y_grid_, phi_grid_);
}

// 构造：使用一段离散轨迹构造节点，节点状态使用轨迹的最后一个点
Node3d::Node3d(const std::vector<double> &traversed_x, // 返回末尾点的索引
               const std::vector<double> &traversed_y,
               const std::vector<double> &traversed_phi,
               const std::vector<double> &XYbounds,
               const PlannerOpenSpaceConfig &open_space_conf) {
  // 这里默认输入合法：traversed_x/y/phi 长度相同且非空

  // 节点的连续状态取轨迹最后一个点
  x_ = traversed_x.back();
  y_ = traversed_y.back();
  phi_ = traversed_phi.back();

  // 根据配置计算网格索引
  x_grid_ = static_cast<int>((x_ - XYbounds[0]) /
                       open_space_conf.warm_start_config.xy_grid_resolution);
  y_grid_ = static_cast<int>((y_ - XYbounds[2]) /
                       open_space_conf.warm_start_config.xy_grid_resolution);
  phi_grid_ = static_cast<int>(
      (phi_ - (-M_PI)) / open_space_conf.warm_start_config.phi_grid_resolution);

  // 保存整段离散轨迹
  traversed_x_ = traversed_x;
  traversed_y_ = traversed_y;
  traversed_phi_ = traversed_phi;

  index_ = ComputeStringIndex(x_grid_, y_grid_, phi_grid_);
  step_size_ = traversed_x.size();
}

// 计算车辆在给定状态下的包围盒（Axis-Aligned Bounding Box 的旋转版本，即 OBB）
Box2d Node3d::GetBoundingBox(const VehicleParam &vehicle_param_, const double x,
                             const double y, const double phi) {
  double ego_length = vehicle_param_.length;
  double ego_width = vehicle_param_.width;
  // shift_distance: 将车辆参考点（通常为后轴中心）转换为车体中心的偏移
  double shift_distance = ego_length / 2.0 - vehicle_param_.back_edge_to_center;
  // 构造 Box2d：box 的中心为 (x + shift*cos(phi), y + shift*sin(phi))
  // 长度和宽度目前未做缩放（若需要膨胀障碍物，可在这里乘以系数）
  Box2d ego_box({x + shift_distance * std::cos(phi),
                 y + shift_distance * std::sin(phi)}, // 中心点坐标
                phi, ego_length * 1, ego_width * 1); // 方向角，长度，宽度
  return ego_box;
}

// 比较两个节点是否相等（通过字符串索引比较）
bool Node3d::operator==(const Node3d &right) const {
  return right.GetIndex() == index_;
}

// 将整数网格坐标拼成字符串索引，格式为 "xGrid_yGrid_phiGrid"
std::string Node3d::ComputeStringIndex(int x_grid, int y_grid, int phi_grid) {
  return absl::StrCat(x_grid, "_", y_grid, "_", phi_grid);
}
